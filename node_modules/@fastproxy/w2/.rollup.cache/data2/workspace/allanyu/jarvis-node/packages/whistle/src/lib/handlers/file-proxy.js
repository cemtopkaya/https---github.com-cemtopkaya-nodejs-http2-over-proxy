import { Buffer, PassThrough, extend, fs, mime, qs } from '../import-pkg';
import * as util from '../util';
import * as protoMgr from '../rules/protocols';
...
export function fileProxy(req, res, next) {
    ...
    const defaultType = mime.lookup(req.fullUrl.replace(/[?#].*$/, ''), 'text/html');
    ...
    if (rule.value) {
        ...
        let reader = isRawFile
            ...
            : {
                ...
                body: body,
                headers: {
                    'content-type': (rule.key ? mime.lookup(rule.key, defaultType) : defaultType) +
                        '; charset=utf-8'
                }
            };
        if (isTpl) {
    ...
    function render(reader) {
        if (reader.body) {
            const data = qs.parse(util.getQueryString(req.fullUrl));

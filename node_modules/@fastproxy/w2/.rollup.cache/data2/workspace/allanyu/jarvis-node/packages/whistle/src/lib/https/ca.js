import { LRU, crypto, extend, forge, fs, hagent, net, path, tls } from '../import-pkg';
import * as util from '../util';
import config from '../config';
import * as h2 from './h2';
export const createSecureContext = tls.createSecureContext || crypto.createCredentials;
const pki = forge.pki;
...
const HTTPS_DIR = mkdir(path.join(config.getDataDir(), 'certs'));
const ROOT_NEW_KEY_FILE = path.join(HTTPS_DIR, 'root_new.key');
const ROOT_NEW_CRT_FILE = path.join(HTTPS_DIR, 'root_new.crt');
export const CUSTOM_CERTS_DIR = config.disableCustomCerts
    ? null
    : config.CUSTOM_CERTS_DIR;
const useNewKey = fs.existsSync(ROOT_NEW_KEY_FILE) && fs.existsSync(ROOT_NEW_CRT_FILE);
...
const customCertDir = config.certDir;
let customPairs = {};
let customCertsInfo = {};
let customCertsFiles = {};
let allCustomCerts = {};
let customCertCount = 0;
const cachePairs = new LRU({ max: 5120 });
const certsCache = new LRU({ max: 256 });
export const remoteCerts = new LRU({ max: 1280 });
...
function checkCertificate() {
    try {
        const crt = pki.certificateFromPem(fs.readFileSync(ROOT_CRT_FILE).toString());
...
export const existsCustomCert = function (hostname) {
    if (!customCertCount) {
        return false;
    }
...
export const hasCustomCerts = function () {
...
export const createCertificate = function (hostname) {
...
function loadCustomCerts(certDir, isCustom) {
    ...
    try {
        fs.readdirSync(certDir).forEach(function (name) {
            ...
            try {
                const filePath = path.join(certDir, name);
                cert.dir = certDir;
...
export const createRootCA = function (opts) {
...
function getOrCreateCert(servername) {
    ...
    let cert = remoteCerts.get(servername);
    if (!cert) {
...
const getHttp2ServerVar = hagent.create(h2.getHttpServer, 42900);
const getHttpsServer = hagent.create(h2.getServer, 43900);
const cbs = {};
const ports = {};
const TIMEOUT = 6000;
export const SNICallback = function (servername, cb) {
...
export const getRootCA = function () {
    return {
        key: rootKey,
        cert: rootCrt
    };
};
export const getCustomCertsInfo = function () {
    return customCertsInfo;
};
export const getCustomCertsFiles = function () {
    return customCertsFiles;
};
export const serverAgent = hagent.serverAgent;
...
export const getHttp2Server = function (listener, callback) {
    const name = 'httpH2';
...
export const getSNIServer = function (listener, callback, disableH2, requestCert) {
    const enableH2 = config.enableH2 && !disableH2;
    ...
    let options = { SNICallback: SNICallback };
...
export var hasInvalidCerts = false;
...
function writeCertFile(filename, cert, mtime) {
    const keyFile = path.join(CUSTOM_CERTS_DIR, filename + '.key');
    const certFile = path.join(CUSTOM_CERTS_DIR, filename + '.crt');
...
export const removeCert = function (filename) {
    if (!CUSTOM_CERTS_DIR) {
        return;
    }
...
export const uploadCerts = function (certs) {
    if (!CUSTOM_CERTS_DIR) {
        return;
    }
    const now = Date.now();

...
export function initMiddleware(req, res, next) {
    ...
    let clientPort = clientInfo[1] || headers[config.CLIENT_PORT_HEAD];
    ...
    const isHttps = req.socket.isHttps || req.isHttps || headers[config.HTTPS_FIELD];
    if (isHttps) {
        req.isHttps = true;
    ...
    const alpn = headers[config.ALPN_PROTOCOL_HEADER];
    if (alpn) {
        if (alpn === 'httpH2') {

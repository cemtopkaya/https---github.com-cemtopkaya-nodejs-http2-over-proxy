import './lib/util/patch';
import createDebug from 'debug';
import { assert, cluster, extend, net, os, path, tls } from './lib/import-pkg';
import config from './lib/config';
import * as common from './lib/util/common';
import whistleProxy from './lib/index';
const debug = createDebug('@w2.index');
export const getWhistlePath = common.getWhistlePath;
...
function loadConfig(options) {
    ...
    if (config) {
        ...
        debug(`loadConfig file: ${path.resolve(config)}`);
...
function forkWorker(index) {
    const worker = cluster.fork({ workerIndex: index });
    let reforked;
    const refork = () => {
        if (!state[index]) {
            setTimeout(function () {
...
export default function (options, callback) {
    ...
    const startWhistle = function () {
        ...
        if (server) {
            assert(options?.port && options?.port > 0, 'options.port of the custom server is required');
        ...
        const conf = config.extend(options);
        // debug('default:: conf=', conf);
        if (!conf.cluster) {
            return whistleProxy(callback, server);
        }
        let timer;

...
import { Buffer, LRU, createAgent, express, extend, hparser, http, https, iconv, path, wsParser } from '../import-pkg';
import { listenerCount } from '../util/patch';
import { request } from '../util/http-mgr';
import isUtf8 from '../util/is-utf8';
import Storage from '../rules/storage';
import parseUrl from '../util/parse-url';
import * as transproto from '../util/transproto';
import * as common from '../util/common';
import getProxy from './proxy';
const debug = createDebug('@w2.lib.plugins.load-plugin');
// 只能如此导入
const rootRequire = require('./require');
const getServer = createAgent(null, 40500);
...
function wrapTunnelWriter(socket, toServer) {
    ...
    const sender = wsParser.getSender(socket, toServer);
...
function wrapTunnelReader(socket, fromServer, maxPayload) {
    socket.wsExts = '';
    const receiver = wsParser.getReceiver(socket, fromServer, maxPayload);
...
function addFrameHandler(req, socket, maxWsPayload, fromClient, toServer) {
    ...
    const receiver = wsParser.getReceiver(socket, !fromClient, maxWsPayload);
    const emit = socket.emit;
    const write = socket.write;
    const end = socket.end;
    let lastOpts;
...
export default async function (options, callback) {
    ...
    let pluginDataDir = (config.pluginBaseDir = path.join(config.baseDir, '.plugins', options.name));
    if (config.storage) {
    ...
    options.isEnable = options.isActive = function (cb) {
        ...
        const checkEnable = function () {
            requestCgi(enableOpts, function (data, _r) {
                if (!data) {
    ...
    const initServers = function (_ctx) {
        ...
        if (staticDir) {
            ...
            startUIServer = function (server) {
                app.use(express.static(path.join(options.value, staticDir), {
                    maxAge: 60000
                }));
    ...
    initProxy(async function (proxy) {
        ...
        let initial = loadModule(path.join(options.value, 'initial.js')) || loadModule(path.join(options.value, 'initialize.js'));

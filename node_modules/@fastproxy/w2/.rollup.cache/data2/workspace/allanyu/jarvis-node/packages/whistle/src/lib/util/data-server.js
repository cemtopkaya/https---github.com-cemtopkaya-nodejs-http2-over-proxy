...
import { getStatus } from '../socket-mgr';
import logger from './logger';
import { encodeURIComponent, trimStr } from './index';
...
export default function init(_proxy) {
    proxy = _proxy;
    enable();
    /**
     * options: {
    ...
    function formatFilter(filter, clientIp, clientId) {
        ...
        const url = trimStr(filter.url).toLowerCase();
        let ip = trimStr(filter.ip);
        const list = [];
        let cid;
        let result = {};
        if (ip === 'self') {
        ...
        for (let i = 0; i < 6; i++) {
            const key = 'name' + (i || '');
            const name = trimStr(filter[key]).toLowerCase();
            if (name) {
                result = result || {};
                const value = trimStr(filter['value' + (i || '')]).toLowerCase();
                if (i) {
                    headers = headers || [];
    ...
    function checkHeader(text, keyword, exact) {
        ...
        if (exact) {
            return (text === keyword || text === encodeURIComponent(keyword).toLowerCase());
        }
        return (text.indexOf(keyword) !== -1 ||
            text.indexOf(encodeURIComponent(keyword).toLowerCase()) !== -1);
    }
    ...
    proxy.getData = function (options, clientIp, key, value, filterClientId, clientId) {
        ...
        return {
            ...
            socketStatus: getStatus(options.curReqId)
        };
    };
}

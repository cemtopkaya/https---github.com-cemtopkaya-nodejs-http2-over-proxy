...
import { extend, hparser, http, https, mime, net, url } from '../import-pkg';
import { FileWriterTransform, ReplacePatternTransform, ReplaceStringTransform, SpeedTransform, WhistleTransform } from '../util';
import * as util from '../util';
import * as h2 from '../https/h2';
...
const TIMEOUT = config.timeout < 16000 && config.timeout > 0 ? 0 : 16000;
...
export function inspectRes(req, res, next) {
    let origProto;
                ...
                rules.getProxy(options.href, options.isPlugin ? null : req, function (_err, hostIp, hostPort) {
                    ...
                    rules.resolveHost(req, function (err, ip, port, hostRule) {
                        ...
                        if (proxyUrl) {
                            ...
                            if (isHttps || (req.useH2 && !isInternalProxy) || isSocks || isHttpsProxy || req._phost) {
                                ...
                                if (isProxyPort && util.isLocalAddress(ip)) {
                                    ...
                                    const clientId = req.headers[config.CLIENT_ID_HEADER];
                                    if (clientId) {
                        ...
                        if (isProxyPort && isLocalAddress) {
                            const redirectHost = config.customLocalUIHost || ip;
                            const redirectPort = config.realPort || config.port;
                            res.response(util.wrapResponse({
                                statusCode: 302,
                                headers: {
                        ...
                        function request(options, serverPort, direct) {
                            ...
                            const send = function (sock) {
                                ...
                                try {
                                    const client = (useHttps ? https : http).request(options, res.response);
                                    curClient = client;
                            ...
                            if (proxyUrl) {
                                ...
                                const clientId = optHeaders[config.CLIENT_ID_HEADER];
                                if (clientId) {
                    ...
                    util.parseRuleJson(ruleList, function (headers, cookies, cors, replacement, params, newTrailers) {
                        ...
                        util.readInjectFiles(data, function (data) {
                            ...
                            util.getRuleValue([
                                ...
                                const speedTransform = data.speed || data.delay ? new SpeedTransform(data) : null;
                                delete data.headers;
                                delete data.speed;
                                delete data.delay;

...
import { getRootCAFile } from '../../../lib/https/ca';
import config from '../../../lib/config';
import { pluginMgr as pluginsMgr } from '../../../lib/plugins';
import handleWeinreReq from '../../weinre';
import * as htdocs from '../htdocs';
const app = express();
const PARSE_CONF = { extended: true, limit: '3mb' };
const UPLOAD_PARSE_CONF = { extended: true, limit: '30mb' };
const urlencodedParser = bodyParser.urlencoded(PARSE_CONF);
const jsonParser = bodyParser.json(PARSE_CONF);
const uploadUrlencodedParser = bodyParser.urlencoded(UPLOAD_PARSE_CONF);
const uploadJsonParser = bodyParser.json(UPLOAD_PARSE_CONF);
...
const assetsDir = (file) => path.resolve(__dirname, '../../../../assets', file);
const MENU_HTML = assetsDir('menu.html');
const INSPECTOR_HTML = assetsDir('tab.html');
const MENU_URL = '???_WHISTLE_PLUGIN_EXT_CONTEXT_MENU_' + config.port + '???';
const INSPECTOR_URL = '???_WHISTLE_PLUGIN_INSPECTOR_TAB_' + config.port + '???';
...
function readRemoteStream(req, res, authUrl) {
    ...
    if (authUrl[0] === 'f') {
        const stream = fs.createReadStream(authUrl.substring(7));
        stream.on('error', handleError);
        return stream.pipe(res);
    }
    const options = url.parse(authUrl);
    options.rejectUnauthorized = false;
    const httpModule = options.protocol === 'https:' ? https : http;
    const headers = extend({}, req.headers);
    delete headers.host;
...
app.use(function (req, res, next) {
    ...
    pluginsMgr.loadAuthPlugins(req, function (status, msg, authUrl) {
        if (!status && !authUrl) {
            return next();
        }
...
app.use(function (req, res, next) {
    ...
    if (index === 0) {
        ...
        if (!pluginName && referer) {
            const refOpts = url.parse(referer);
...
async function cgiHandler(req, res) {
    ...
    const filepath = path.join(__dirname, '../' + req.path) + '.js';
    // debug(`:cgiHandler filepath=${filepath}`);
    const handleResponse = async function () {
        try {
            const middle = filepath.endsWith('.ts') ? await import(filepath) : require(filepath);
            (middle.default || middle)(req, res);
        }
        catch (err) {
            const msg = config.debugMode ? '<pre>' + util.getErrorStack(err) + '</pre>' : 'Internal Server Error';
...
function readPluginPage(req, res, plugin, html, config) {
    ...
    if (index === -1) {
        ...
        let reader = fs.createReadStream(path.join(plugin.path, filepath));
...
app.use(function (req, res, next) {
    if (ALLOW_PLUGIN_PATHS.indexOf(req.path) !== -1) {
        const name = req.headers[config.PROXY_ID_HEADER];
        if (name) {
    ...
    const guestAuthKey = config.guestAuthKey;
...
if (!config.debugMode) {
    const indexHtml = fs.readFileSync(htdocs.getHtmlFile('index.html'));
    const indexJs = fs.readFileSync(htdocs.getJsFile('index.js'));
    const jsETag = shasum(indexJs);
    const gzipIndexJs = zlib.gzipSync(indexJs);
...
export const getProxy = () => proxyEvent;
app.use(express.static(htdocs.htdocsRoot(), { maxAge: 300000 }));
export const setupServer = function (server) {

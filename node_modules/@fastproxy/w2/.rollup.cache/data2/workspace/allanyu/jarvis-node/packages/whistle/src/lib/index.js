...
import { initBiz } from '../biz/init';
import { bizMiddleware } from '../biz';
import { EventEmitter, express, extend, http, https, socks } from './import-pkg';
import * as util from './util';
import logger from './util/logger';
import * as rules from './rules';
import { setup as setupHttps } from './https';
import * as httpsUtil from './https/ca';
import * as rulesUtil from './rules/util';
import initDataServer from './util/data-server';
import initLogServer from './util/log-server';
import { pluginMgr } from './plugins';
import config from './config';
import loadService from './service';
import initSocketMgr from './socket-mgr';
import tunnelProxy from './tunnel';
import upgradeProxy from './upgrade';
import proc from './util/process';
import * as perf from './util/perf';
import loadCert from './https/load-cert';
import * as common from './util/common';
import { initMiddleware } from './init';
import { inspectMiddlewares } from './inspectors';
import { handlerMiddlewars } from './handlers';
const debug = createDebug('@w2.lib.index');
...
function proxy(callback, _server) {
    const app = express();
    const server = _server || http.createServer();
    const middlewares = [initMiddleware, bizMiddleware, ...inspectMiddlewares]
        .concat(config.middlewares)
        .concat(handlerMiddlewars);
    ...
    if (config.socksPort) {
        ...
        const socksServer = socks.createServer(function (info, accept, deny) {
            ...
            if (config.socksMode ||
                (dstPort != 80 &&
                    dstPort != 443 &&
                    (dstPort != config.port || (!util.isLocalAddress(dstAddr) && !config.isLocalUIUrl(dstAddr))))) {
        ...
        util.getBoundIp(config.socksHost, (host) => {
...
config.addInitHook(() => rulesUtil.parseRules());
export default exportInterfaces(proxy);

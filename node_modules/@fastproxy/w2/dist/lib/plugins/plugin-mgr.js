...
exports.pluginMgr = exports.PluginMgr = void 0;
const tslib_1 = require("tslib");
const safe_1 = tslib_1.__importDefault(require("colors/safe"));
const debug_1 = tslib_1.__importDefault(require("debug"));
const pfork_1 = require("@fastproxy/pfork");
const import_pkg_1 = require("../import-pkg");
const util = tslib_1.__importStar(require("../util"));
const logger_1 = tslib_1.__importDefault(require("../util/logger"));
const config_1 = tslib_1.__importDefault(require("../config"));
const rulesMgr = tslib_1.__importStar(require("../rules"));
const rules_1 = tslib_1.__importDefault(require("../rules/rules"));
const util_1 = require("../rules/util");
const httpMgr = tslib_1.__importStar(require("../util/http-mgr"));
const protocols = tslib_1.__importStar(require("../rules/protocols"));
const get_plugins_1 = tslib_1.__importDefault(require("./get-plugins"));
const get_plugins_sync_1 = tslib_1.__importDefault(require("./get-plugins-sync"));
const pluginUtil = tslib_1.__importStar(require("./util"));
const debug = (0, debug_1.default)('@w2.lib.plugins.plugin-mgr');
...
const PLUGIN_MAIN = import_pkg_1.path.join(__dirname, './load-plugin');
...
const GLOBAL_PLUGIN_VARS_HEAD = 'x-whistle-global-plugin-vars' + config_1.default.uid;
const PLUGIN_VARS_HEAD = 'x-whistle-plugin-vars' + config_1.default.uid;
...
const SHOW_LOGIN_BOX = 'x-whistle2-show-login-box.' + config_1.default.uid;
const FROM_TUNNEL_HEADER = 'x-whistle-from-tunnel-req-' + config_1.default.uid;
const CLIENT_PORT_HEAD = config_1.default.CLIENT_PORT_HEAD;
...
const rulesCache = new import_pkg_1.LRU({ max: 36 });
const CUSTOM_CERT_HEADER = config_1.default.CUSTOM_CERT_HEADER;
const ENABLE_CAPTURE_HEADER = config_1.default.ENABLE_CAPTURE_HEADER;
const PLUGIN_HOOKS = config_1.default.PLUGIN_HOOKS;
const PLUGIN_HOOK_NAME_HEADER = config_1.default.PLUGIN_HOOK_NAME_HEADER;
const HTTP_RE = /^https?:\/\//;
const conf = {};
...
const debugMode = config_1.default.debugMode;
let whistleProxy;
const getPlugin = (cb) => {
    const notLoadPlugins = config_1.default.networkMode || config_1.default.rulesOnlyMode;
    notLoadPlugins ? cb({}) : (0, get_plugins_1.default)(cb);
};
config_1.default.addInitHook(() => {
    const notLoadPlugins = config_1.default.networkMode || config_1.default.rulesOnlyMode;
    allPlugins = notLoadPlugins ? {} : (0, get_plugins_sync_1.default)();
    ...
    conf.PLUGIN_HOOKS = config_1.default.PLUGIN_HOOKS;
    conf.uiHostList = config_1.default.uiHostList;
    const pluginHostMap = config_1.default.pluginHostMap;
...
class PluginMgr extends import_pkg_1.EventEmitter {
    constructor() {
        super();
        this.getTunnelKeys = () => {
            return tunnelKeys;
        ...
        this.loadCert = (req, plugin, callback) => {
            this.loadPlugin(plugin, (err, ports) => {
                ...
                if (!req.useSNI || req.isHttpsServer) {
                    options.headers[config_1.default.SNI_TYPE_HEADER] = req.isHttpsServer ? '1' : '0';
                }
                if (req.sniRuleValue) {
        ...
        this.loadPlugin = (plugin, callback) => {
            ...
            util.getBoundIp(config_1.default.host, (host) => {
                conf.host = host || LOCALHOST;
                const moduleName = plugin.moduleName;
                const name = moduleName.substring(moduleName.indexOf('/') + 1);
                const isInline = config_1.default.inspectMode || process.env.PFORK_MODE === 'inline';
                (0, pfork_1.fork)({
                    data: config_1.default.getPluginData(moduleName),
                    _inspect: config_1.default.inspectMode,
                    name: moduleName,
                    script: PLUGIN_MAIN,
                    value: plugin.path,
                    ...
                    CLIENT_IP_HEADER: config_1.default.CLIENT_IP_HEAD,
                    ...
                    debugMode: debugMode,
                    config: isInline ? (0, import_pkg_1.extend)(true, {}, conf) : conf // 防止 inline 时，子进程删除 conf
        ...
        this.isDisabled = (name) => {
            if (config_1.default.notAllowedDisablePlugins) {
                return false;
            }
            if (util_1.properties.get('disabledAllPlugins')) {
                return true;
            }
            const disabledPlugins = util_1.properties.get('disabledPlugins') || {};
        ...
        this.loadAuthPlugins = (req, callback) => {
            ...
            this._loadPlugins(authPlugins, (ports) => {
                ...
                authReq(true, ports, req, options, (forbidden) => {
                    if (!config_1.default.disableAuthUI && forbidden) {
        ...
        this.postStats = (req, res) => {
            ...
            this.loadPlugins(plugins, (ports) => {
                ...
                ports.forEach((item) => {
                    const opts = (0, import_pkg_1.extend)({}, options);
                    opts.headers = (0, import_pkg_1.extend)({}, options.headers);
                    opts.port = item.port;
                    addPluginVars(req, opts.headers, item);
                    const request = import_pkg_1.http.request(opts, (response) => {
        ...
        this.updatePluginRules = (name) => {
            name && httpMgr.forceUpdate('http://127.0.0.1:' + config_1.default.port + '/whistle.' + name + '/');
        };
        this.setProxy = (p) => {
            whistleProxy = p;
        };
        ...
        this.getResRules = (req, res, callback) => {
            ...
            config_1.default.multiEnv && util.mergeRules(req, resRules, true);
            hRules && util.mergeRules(req, hRules, true);
            pRules && util.mergeRules(req, pRules, true);
            !config_1.default.multiEnv && util.mergeRules(req, resRules, true);
            let resScriptRules;
            let resHeaderRules = res.headers[config_1.default.RES_RULES_HEAD];
            if (resHeaderRules) {
                try {
            ...
            delete res.headers[config_1.default.RES_RULES_HEAD];
            ...
            this.loadPlugins(plugins, (ports) => {
                ...
                authReq(enableAuth, ports, req, options, (forbidden) => {
                    if (forbidden) {
                        ...
                        const mgr = new rules_1.default({ msg: forbidden });
                        ...
                        const handleRules = (err, body, values, raw, res) => {
                            ...
                            if (body || values) {
                                ...
                                if (values && pluginVals) {
                                    const vals = (0, import_pkg_1.extend)({}, pluginVals);
                                    values = (0, import_pkg_1.extend)(vals, values);
                                }
                                else {
        ...
        this.resolveKey = (url, rule, req) => {
            ...
            const headers = (0, import_pkg_1.extend)({}, req && req.headers, config_1.default.pluginHeaders);
            if (req) {
        ...
        this.getPipe = (type, hookName) => {
            ...
            return (req, res, callback) => {
                ...
                this.resolvePipePlugin(req, (ports) => {
                    ...
                    client = config_1.default.connect(options, handleConnect);
                    client.on('error', () => {
                        if (client) {
                            destroy();
                            client = !req._hasError && config_1.default.connect(options, handleConnect);
        ...
        this.getOptions = (req, res, type, isPipe) => {
            ...
            const headers = (0, import_pkg_1.extend)({}, isResRules ? res.headers : req.headers);
            delete headers.upgrade;
            delete headers.connection;
...
function showVerbose(oldData, newData) {
    ...
    uninstallData &&
        Object.keys(uninstallData).forEach(name => {
            console.log(safe_1.default.red(util.formatDate(new Date(uninstallData[name].mtime)) + ' [uninstall plugin] ' + name.slice(0, -1)));
        });
    installData &&
        Object.keys(installData).forEach(name => {
            console.log(safe_1.default.green(util.formatDate(new Date(installData[name].mtime)) + ' [install plugin] ' + name.slice(0, -1)));
        });
    updateData &&
        Object.keys(updateData).forEach(name => {
            console.log(safe_1.default.yellow(util.formatDate(new Date(updateData[name].mtime)) + ' [update plugin] ' + name.slice(0, -1)));
        });
}
function readFile(filepath, callback) {
    import_pkg_1.fs.readFile(filepath, UTF8_OPTIONS, (err, text) => {
        if (!err) {
            return callback(err, text);
        }
        import_pkg_1.fs.readFile(filepath, UTF8_OPTIONS, callback);
    });
}
function readReqRules(dir, callback) {
    readFile(import_pkg_1.path.join(import_pkg_1.path.join(dir, '_rules.txt')), (err, rulesText) => {
        if (err) {
            readFile(import_pkg_1.path.join(import_pkg_1.path.join(dir, 'reqRules.txt')), (_, rulesText) => {
...
function readJson(pkgPath, callback) {
    import_pkg_1.fse.readJson(pkgPath, (err, json) => {
        if (!err) {
            return callback(err, json);
        }
        import_pkg_1.fse.readJson(pkgPath, callback);
    });
}
function readPackages(obj, callback) {
    ...
    Object.keys(obj).forEach(name => {
        ...
        if (!pkg || pkg.path != newPkg.path || pkg.mtime != newPkg.mtime) {
            ++count;
            readJson(import_pkg_1.path.join(newPkg.path, 'package.json'), (_, result) => {
                if (result && result.version && pluginUtil.isPluginName(result.name)) {
                    ...
                    readFile(import_pkg_1.path.join(import_pkg_1.path.join(newPkg.path, 'rules.txt')), (_err, rulesText) => {
                        ...
                        readReqRules(newPkg.path, (rulesText) => {
                            ...
                            readFile(import_pkg_1.path.join(import_pkg_1.path.join(newPkg.path, '_values.txt')), (_err, rulesText) => {
                                newPkg[util.PLUGIN_VALUES] = pluginUtil.parseValues(util.renderPluginRules(rulesText, result, simpleName));
                                readFile(import_pkg_1.path.join(import_pkg_1.path.join(newPkg.path, 'resRules.txt')), (_err, rulesText) => {
            ...
            getPlugin((result) => {
                readPackages(result, (_plugins) => {
                    ...
                    if (uninstallPlugins || updatePlugins || Object.keys(_plugins).length !== pluginNames.length) {
                        uninstallPlugins && exports.pluginMgr.emit('uninstall', uninstallPlugins);
                        updatePlugins && exports.pluginMgr.emit('update', updatePlugins);
                        exports.pluginMgr.emit('updateRules');
                    }
                    update();
                });
            });
...
function addPluginHeaders(req, headers, isKey) {
    ...
    headers[isKey ? 'x-whistle-remote-address' : config_1.default.REMOTE_ADDR_HEAD] = req._remoteAddr || LOCALHOST;
    headers[isKey ? 'x-whistle-remote-port' : config_1.default.REMOTE_PORT_HEAD] = req._remotePort || '0';
    if (req.fromComposer) {
        headers[config_1.default.REQ_FROM_HEADER] = 'W2COMPOSER';
    }
    else {
        delete headers[config_1.default.REQ_FROM_HEADER];
    }
    return headers;
}
function parseRulesList(req, results, isResRules) {
    ...
    results.reverse().forEach((item) => {
        (0, import_pkg_1.extend)(values, item.values);
    });
    const pluginRulesMgr = new rules_1.default(values);
    ...
    ports.forEach((item) => {
        ...
        requestPlugin(opts, (err, body, res) => {
            ...
            if (err || body) {
                ...
                err && logger_1.default.error(err);
            }
            if (!forbidden && headers) {
                Object.keys(headers).forEach(key => {
                    if (key.indexOf('x-whistle-') === 0 || key === 'proxy-authorization') {
                        const value = headers[key];
                        if (key === config_1.default.WHISTLE_POLICY_HEADER && value === 'enableCaptureByAuth') {
...
function requestValue(options, callback, isBin) {
    ...
    httpMgr.request((0, import_pkg_1.extend)({}, options), (err, body, res) => {
        if (err) {
...
exports.pluginMgr = new PluginMgr();
httpMgr.setPluginMgr(exports.pluginMgr);
util.setPluginMgr(exports.pluginMgr);
httpMgr.addChangeListener(exports.pluginMgr.updateRules);
rulesMgr.setGetPluginFn(exports.pluginMgr.getActivePluginByName);

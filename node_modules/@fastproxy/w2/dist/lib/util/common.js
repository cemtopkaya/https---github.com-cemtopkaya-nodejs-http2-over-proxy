...
exports.isGroup = exports.wrapRuleValue = exports.encodeHtml = exports.writeLogSync = exports.getLogFile = exports.getWhistlePath = exports.getHomePath = exports.getHomedir = exports.replaceProtocol = exports.removeProtocol = exports.getProtocol = exports.setProtocol = exports.hasProtocol = exports.isConnect = exports.isWebSocket = exports.isUpgrade = exports.onResEnd = exports.addTrailerNames = exports.removeIllegalTrailers = exports.lowerCaseify = exports.isEmptyObject = exports.hasBody = exports.removeIPV6Prefix = exports.noop = exports.getUpdateUrl = exports.isHead = exports.REMOTE_URL_RE = void 0;
...
exports.REMOTE_URL_RE = /^\s*((?:git[+@]|github:)[^\s]+\/whistle\.[a-z\d_-]+(?:\.git)?|https?:\/\/[^\s]+)\s*$/i;
function isHead(req) {
    return HEAD_RE.test(req.method);
}
exports.isHead = isHead;
const getUpdateUrl = function (conf) {
    const url = conf.updateUrl;
    if (url && exports.REMOTE_URL_RE.test(url) && url.length < 2100) {
        return url;
    }
};
...
const lowerCaseify = function (obj, rawNames) {
    const result = {};
    if (!obj) {
        return result;
    }
...
const removeIllegalTrailers = function (headers) {
    ILLEGAL_TRAILERS.forEach(function (key) {
        delete headers[key];
    });
};
exports.removeIllegalTrailers = removeIllegalTrailers;
const addTrailerNames = function (res, newTrailers, rawNames, delTrailers, req) {
...
exports.addTrailerNames = addTrailerNames;
const onResEnd = function (res, callback) {
...
const isUpgrade = function (options, headers) {
...
function getHomedir() {
    ...
    return ((typeof import_pkg_1.os.homedir === 'function'
        ? import_pkg_1.os.homedir()
...
function getHomePath(dir) {
    ...
    return import_pkg_1.path.join(getHomedir(), '.' + dir.substring(1));
}
exports.getHomePath = getHomePath;
function getWhistlePath() {
    return (getHomePath(process.env.WHISTLE_PATH) ||
        import_pkg_1.path.join(getHomedir(), '.WhistleAppData'));
}
exports.getWhistlePath = getWhistlePath;
function getLogFile(name) {
    const whistlePath = getWhistlePath();
    import_pkg_1.fse.ensureDirSync(whistlePath);
    return import_pkg_1.path.join(whistlePath, import_pkg_1.pkgInfo.name.replace('/', '-') + (name ? '-' + name : '') + '.log');
}
...
const writeLogSync = function (msg) {
    try {
        import_pkg_1.fs.writeFileSync(getLogFile(), msg, { flag: 'a' });
    }
    catch (e) {
        msg += '\r\n' + e.stack;
        try {
            import_pkg_1.fs.writeFileSync(getLogFile('error'), msg, { flag: 'a' });
        }
        catch (e) {
            import_pkg_1.fs.writeFileSync(getLogFile(getDate()), msg + '\r\n' + e.stack, {
                flag: 'a'
            });
        }
    }
...
const SEP_RE_G = /^[^\n\r\S]*```+\s*$/gm;
const wrapRuleValue = function (key, value, size, policy) {
    if (!key || value == null) {
        return '';
    }
...
exports.wrapRuleValue = wrapRuleValue;

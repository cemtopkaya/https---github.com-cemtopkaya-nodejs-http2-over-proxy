...
exports.sendData = exports.changeStatus = exports.getData = exports.exists = exports.setPending = exports.removePending = exports.getStatus = exports.abort = exports.handleConnect = exports.handleUpgrade = exports.removeContext = exports.setContext = void 0;
...
const conns = {};
const PING = safe_buffer_1.Buffer.from('iQA=', 'base64');
const PONG = safe_buffer_1.Buffer.from('ioAn6ubf', 'base64');
...
function handleSocketEnd(req, res, callback) {
    (0, util_1.onSocketEnd)(req, callback);
    (0, util_1.onSocketEnd)(res, callback);
}
function handleClose(req, res, justTunnel) {
    handleSocketEnd(req, res, function (err) {
        ...
        if (req.customParser) {
            !closed && (0, exports.removePending)(req.reqId);
            return;
        }
...
function isHide(req) {
    return !config_1.default.captureData || (req._filters.hide && !req.disable.hide);
}
...
function drainData(status, socket, receiver, toServer) {
    if (!status.sender) {
        try {
            status.sender = ws_parser_1.default.getSender(socket.pipeWriter || socket, toServer);
        }
        catch (e) { }
    }
    status.sender &&
...
function handleWsSend(ctx, reqTrans, sendStatus, isTunnel) {
    ...
    if (!hideWs) {
        try {
            reqReceiver = ws_parser_1.default.getReceiver(res);
        }
        catch (e) {
            hideWs = true;
        }
    ...
    (0, util_1.onSocketEnd)(res, function () {
...
function handleWsReceive(ctx, resTrans, receiveStatus, isTunnel) {
    ...
    if (!hideWs) {
        try {
            resReceiver = ws_parser_1.default.getReceiver(res, true);
        }
        catch (e) {
            hideWs = true;
        }
    ...
    const reqId = req.reqId;
    (0, util_1.onSocketEnd)(req, function () {
...
function getContext(req, res, hasEvent, sendStatus, receiveStatus) {
    ...
    const ctx = (conns[reqId] = {
        ...
        hasEvent: hasEvent,
        url: req.fullUrl,
        charset: (0, util_1.getCharset)(res.headers['content-type']) || '',
        clearup: function () {
...
const removeContext = function (req) {
    delete conns[req.reqId];
};
exports.removeContext = removeContext;
const handleUpgrade = function (req, res) {
    if (req.isPluginReq) {
    ...
    const reqTrans = new stream_1.PassThrough();
    const resTrans = new stream_1.PassThrough();
    reqTrans.on('error', emitError);
    resTrans.on('error', emitError);
    res.headers = res.headers || {};
    req.wsExts = res.headers['sec-websocket-extensions'] || '';
    handleClose(req, res);
    plugins_1.pluginMgr.getWsPipe(req, res, function (reqRead, reqWrite, resRead, resWrite) {
        customParser && removePendingFn(reqId);
        if (req._hasClosed) {
            return;
        }
        const hasEvent = (0, util_1.listenerCount)(proxy, 'wsRequest');
...
const handleConnect = function (req, res) {
    const hasEvent = (0, util_1.listenerCount)(proxy, 'tunnelRequest');
    ...
    plugins_1.pluginMgr.getTunnelPipe(req, res, function (reqRead, reqWrite, resRead, resWrite) {
        customParser && removePendingFn(reqId);
        if (req._hasClosed) {
            return;
        }
...
exports.abort = abort;
const getStatus = function (reqId) {
...
const removePending = function (req) {
    removePendingFn(req.reqId);
};
exports.removePending = removePending;
const setPending = function (req) {
...
const exists = function (reqId) {
    return reqId && conns[reqId];
};
exports.exists = exists;
const getData = function (reqId) {
    const ctx = reqId && conns[reqId];
    if (ctx) {
        const result = {
            sendStatus: ctx.sendStatus,
...
function getBuffer(data, charset) {
    if (data.base64) {
        try {
            return safe_buffer_1.Buffer.from(data.base64, 'base64');
        }
        catch (e) { }
    }
    else if (data.text) {
        return (0, util_1.toBuffer)(data.text, charset);
    }
    return;
}

...
exports.setup = exports.addValues = exports.addRules = exports.properties = exports.values = exports.rules = exports.parseRules = void 0;
...
let history = [];
config_1.default.addInitHook(({ LOCAL_FILES }) => {
    try {
        const _files = import_pkg_1.fs.readdirSync(LOCAL_FILES).filter(checkFilename);
        for (let i = 0, len = _files.length; i < len; i++) {
            const _name = _files[i];
            try {
                const stat = import_pkg_1.fs.statSync(import_pkg_1.path.join(LOCAL_FILES, _name));
                if (stat.isFile()) {
                    uploadFiles.push({
                        name: _name,
...
const getRulesStorage = () => {
    if (!_rulesStorage) {
        _rulesStorage = new storage_1.default(config_1.default.rulesDir, { Default: true }, config_1.default.disableWebUI);
    }
...
const getValuesStorage = () => {
    if (!_valuesStorage)
        _valuesStorage = new storage_1.default(config_1.default.valuesDir, undefined, config_1.default.disableWebUI);
    return _valuesStorage;
};
let _propertiesStorage;
const getPropertiesStorage = () => {
    if (!_propertiesStorage) {
        _propertiesStorage = new storage_1.default(config_1.default.propertiesDir, undefined, config_1.default.disableWebUI);
...
function parseRules() {
    const disableRules = !config_1.default.notAllowedDisableRules &&
        getPropertiesStorage().getProperty('disabledAllRules');
    const shadowRules = disableRules && config_1.default.allowDisableShadowRules ? null : config_1.default.shadowRules;
    let value = [];
    if (!disableRules && !config_1.default.multiEnv) {
    ...
    const backRulesFirst = !config_1.default.disabledBackOption &&
    ...
    (0, index_1.parse)(rulesText, null, inlineValues);
    inlineValues = null;
}
...
function selectRulesFile(file) {
    if (!getRulesStorage().existsFile(file) || config_1.default.multiEnv) {
        return;
    }
...
function unselectRulesFile(file, force) {
    if (!force && config_1.default.multiEnv) {
        return;
    }
...
function allowMultipleChoice() {
    return (!config_1.default.disabledMultipleOption &&
...
function moveRulesTo(fromName, toName, clientId) {
    if (getRulesStorage().moveTo(fromName, toName)) {
        ...
        config_1.default.setModified(clientId, true);
...
exports.values = {
    ...
    download: function (name, res) {
        ...
        res.download(import_pkg_1.path.join(config_1.default.LOCAL_FILES, name), name);
    },
    ...
    addUploadFile: function (options, callback) {
        ...
        if (headers && typeof headers === 'string') {
            try {
                content = import_pkg_1.Buffer.from(headers + '\r\n\r\n');
            }
            catch (e) {
                return callback();
            }
        ...
        if (base64 && typeof base64 === 'string') {
            try {
                base64 = import_pkg_1.Buffer.from(base64, 'base64');
                content = content ? import_pkg_1.Buffer.concat([content, base64]) : base64;
            }
            catch (e) {
                return callback();
            }
        ...
        import_pkg_1.fs.writeFile(import_pkg_1.path.join(config_1.default.LOCAL_FILES, name), content.toString(), function (err) {
            if (!err) {
    ...
    removeUploadFile: function (name, callback) {
        ...
        import_pkg_1.fs.unlink(import_pkg_1.path.join(config_1.default.LOCAL_FILES, name), function (err) {
            const index = indexOfUploadFiles(name);
    ...
    moveTo: function (fromName, toName, clientId) {
        if (getValuesStorage().moveTo(fromName, toName)) {
            config_1.default.setModified(clientId);
...
setTimeout(function getWhistleVersion() {
    util.getLatestVersion(config_1.default.registry, function (ver) {
...
const addRules = function (rules, replace, clientId) {
    if (rules == null) {
        return;
    }
...
const addValues = function (values, replace, clientId) {

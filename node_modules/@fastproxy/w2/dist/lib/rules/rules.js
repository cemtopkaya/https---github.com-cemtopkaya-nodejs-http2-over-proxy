...
const rulesUtil = tslib_1.__importStar(require("./util"));
const dns_1 = tslib_1.__importDefault(require("./dns"));
const protoMgr = tslib_1.__importStar(require("./protocols"));
...
function isRegUrl(url, isCheck) {
    ...
    if (result) {
        return isCheck ? result : (0, import_pkg_1.extend)({}, result);
    }
    const oriUrl = url;
...
function resolveVarValue(req, escape, name, key) {
    ...
    switch (lname) {
        ...
        case 'host':
            return config_1.default.host || '';
        case 'realport':
            return config_1.default.realPort || config_1.default.port;
        case 'realhost':
            return config_1.default.realHost || config_1.default.host || '';
        case 'version':
        ...
        case 'localclientid':
            return config_1.default.clientId;
        case 'statuscode':
        case 'status':
    ...
    for (let i = 0; (rule = list[i]); i++) {
        ...
        if (rule.isRegExp) {
            ...
            if (matchedRes && checkFilter() && --index < 0) {
                ...
                files = getFiles(matcher);
                result = (0, import_pkg_1.extend)({ url: matcher, files: files }, rule);
...
function parseRule(rulesMgr, pattern, matcher, raw, root, options) {
    ...
    if (useRealPort) {
        rule.realPort = config_1.default.realPort;
        rule.matcher = rule.matcher.replace('realPort', config_1.default.realPort || config_1.default.port);
    }
    if (proxyName) {
        switch (proxyName) {
            case 'socks':
...
function parseHost(item) {
    ...
    if (IPV4_RE.test(item)) {
        port = port || RegExp.$2;
        item = RegExp.$1;
        if (!import_pkg_1.net.isIP(item)) {
            return false;
        }
    }
    else if (!import_pkg_1.net.isIP(item)) {
        return false;
    }
    return {
        host: item,
...
function resolveMatchFilter(list) {
    ...
    list.forEach((matcher) => {
        if (LINE_PROPS_RE.test(matcher)) {
            ...
            (0, import_pkg_1.extend)(lineProps, util.parseLineProps(matcher));
            return;
        }
        let filter;
        let not;
        ...
        if (PROPS_FILTER_RE.test(matcher) || PURE_FILTER_RE.test(matcher)) {
            ...
            if (isClientPort || isServerPort) {
                ...
                pattern = util.toRegExp(value);
                if (!pattern && !import_pkg_1.net.isIP(value)) {
                    return;
                }
                if (isIp) {
...
const ignoreHost = (req, rulesMgr, filter) => {
    ...
    (0, import_pkg_1.extend)(filter, ignore);
    return (ignore.host ||
        ignore.hosts ||
        ignore['ignore|host'] ||
        ignore['ignore|hosts']);
...
function mergeRule(rules, list, name) {
    if (list.length) {
        if (rules[name]) {
            ...
            rules[name] = (0, import_pkg_1.extend)({ list: list }, list[0]);
        }
    }
}
...
class Rules {
    constructor(values) {
        this.parse = (...args) => {
            ...
            inlineValues = _inlineValues ? (0, import_pkg_1.extend)({}, _inlineValues) : null;
            ...
            if (inlineValues) {
                this._values = (0, import_pkg_1.extend)({}, this._orgValues, inlineValues);
                inlineValues = null;
            }
            else {
        ...
        this.clearAppend = () => {
            if (this._rawText && this._rawText[0].first) {
                ...
                inlineValues = this._inlineValues ? (0, import_pkg_1.extend)({}, this._inlineValues) : null;
        ...
        this.append = (text, root) => {
            ...
            if (this._rawText) {
                ...
                if (inlineValues) {
                    (0, import_pkg_1.extend)(this._values, inlineValues);
                    inlineValues = null;
                }
            }
            else {
        ...
        this.lookupHost = (req, callback) => {
            ...
            const options = import_pkg_1.url.parse(req.curUrl);
            (0, dns_1.default)(options.hostname, callback, allowDnsCache && !this.resolveDisable(req).dnsCache);
        };

...
const recycle_bin_1 = tslib_1.__importDefault(require("./recycle-bin"));
...
class Storage {
    constructor(dir, filters = {}, disabled) {
        ...
        this._writeProperties = () => {
            ...
            import_pkg_1.fse.outputJson(this._properties, this._cache.properties, (err) => {
                this._writePropertiesPending = false;
                if (err) {
        ...
        this._writeFile = (file) => {
            ...
            file._pending = true;
            import_pkg_1.fs.writeFile(this._getFilePath(file), file.data, (err) => {
                file._pending = false;
                if (err) {
        ...
        this._getFilePath = (file, backup) => {
            ...
            return import_pkg_1.path.join(backup ? this._backupDir : this._files, name);
        };
        this.count = () => {
        ...
        this.removeFile = (file) => {
            ...
            import_pkg_1.fs.unlink(this._getFilePath(file), (err) => {
                if (!err) {
                    import_pkg_1.fs.unlink(this._getFilePath(file, true), noop);
                }
            });
            this._writeProperties();
            return true;
        ...
        this.renameFile = (file, newFile) => {
            ...
            import_pkg_1.fs.rename(path, this._getFilePath(file), (err) => {
                if (!err) {
                    import_pkg_1.fs.rename(backupPath, this._getFilePath(file, true), noop);
                }
            }); //不考虑并发
            this._writeProperties();
            return true;
        ...
        this.recycleBin = new recycle_bin_1.default(import_pkg_1.path.join(dir, '.recycle_bin'));
        const backupDir = import_pkg_1.path.join(dir, '.backup');
        import_pkg_1.fse.ensureDirSync(dir);
        import_pkg_1.fse.ensureDirSync(backupDir);
        this._disabled = disabled === true;
        this._backupDir = backupDir;
        this._files = import_pkg_1.path.join(dir, 'files');
        this._properties = import_pkg_1.path.join(dir, 'properties');
        this._backupProps = import_pkg_1.path.join(backupDir, 'properties');
        import_pkg_1.fse.ensureDirSync(this._files);
        import_pkg_1.fse.ensureFileSync(this._properties);
        let maxIndex = -1;
        const files = {};
        const fileNames = { properties: true };
        import_pkg_1.fs.readdirSync(this._files).forEach((file) => {
            if (!/^(\d+)\.(.+)$/.test(file)) {
                return;
            }
            ...
            const filePath = import_pkg_1.path.join(this._files, file);
            if (files[filename]) {
                return import_pkg_1.fs.unlinkSync(filePath);
            }
            if (index > maxIndex) {
                maxIndex = index;
            }
            let data = readFileSafe(filePath);
            const backFile = import_pkg_1.path.join(backupDir, file);
            if (data) {
                import_pkg_1.fs.writeFileSync(backFile, data);
            }
            else {
                data = readFileSafe(backFile);
                if (data) {
                    import_pkg_1.fs.writeFileSync(filePath, data);
                }
            }
            files[filename] = {
                index: index,
            ...
            if (file !== newFile) {
                import_pkg_1.fs.writeFileSync(import_pkg_1.path.join(this._files, newFile), data);
                import_pkg_1.fs.writeFileSync(import_pkg_1.path.join(backupDir, newFile), data);
                import_pkg_1.fs.unlinkSync(filePath);
            }
        });
        let properties = readJsonSafe(this._properties);
        if (properties) {
            import_pkg_1.fse.outputJsonSync(this._backupProps, properties);
        }
        else {
            properties = readJsonSafe(this._backupProps);
            if (properties) {
                import_pkg_1.fse.outputJsonSync(this._properties, properties);
            }
            else {
                properties = {};
            }
        ...
        import_pkg_1.fs.readdirSync(backupDir).forEach((file) => {
            if (fileNames[file]) {
                return;
            }
            try {
                import_pkg_1.fs.unlinkSync(import_pkg_1.path.join(backupDir, file));
            }
            catch (e) { }
        });
    }
...
function copyFile(src, dest, callback, retry) {
    ...
    if (typeof import_pkg_1.fs.copyFile === 'function') {
        import_pkg_1.fs.copyFile(src, dest, execCb);
    }
    else {
        import_pkg_1.fse.copy(src, dest, execCb);
    }
}

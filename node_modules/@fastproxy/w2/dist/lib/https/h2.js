...
exports.request = exports.getHttpServer = exports.getServer = void 0;
...
const clients = {};
const notH2 = new import_pkg_1.LRU({ max: 2560 });
const pendingH2 = {};
const pendingList = {};
const TIMEOUT = 36000;
...
function getSocksSocket(options, callback) {
    ...
    const proxyOpts = options._proxyOptions;
    const client = import_pkg_1.socks.connect({
        localDNS: false,
        proxyHost: proxyOpts.proxyHost,
        proxyPort: proxyOpts.proxyPort,
        auths: config_1.default.getAuths(proxyOpts),
        host: options.host,
...
function getTunnelSocket(options, callback) {
    ...
    const connReq = config_1.default.connect(options._proxyOptions, function (socket) {
        handleCallback(null, socket);
    });
...
function getProxySocket(options, callback, ciphers, isHttp) {
    const handleConnect = function (err, socket) {
        ...
        try {
            socket = import_pkg_1.tls.connect(addCert({
...
function getClient(req, socket, name, callback) {
    ...
    const client = import_pkg_1.http2.connect(origin, {
    ...
    try {
        ...
        h2Session.on('response', function (h2Headers) {
            ...
            if (!util.hasBody(svrRes, req)) {
                h2Session.destroy();
                svrRes = new import_pkg_1.PassThrough();
                svrRes.statusCode = statusCode;
                svrRes.push(null);
            }
            ...
            if (req.isPluginReq && !req._isProxyReq) {
                newHeaders[config_1.default.PROXY_ID_HEADER] = 'h2';
            }
            res.response(svrRes);
        });
        req.pipe(h2Session);
...
const getServer = function (options, listener) {
    let server;
    if (options.allowHTTP1 && import_pkg_1.http2) {
        options.maxSessionMemory = 128;
        options.settings = H2_SVR_SETTINGS;
        server = import_pkg_1.http2.createSecureServer(options);
    }
    else {
        server = import_pkg_1.https.createServer(options);
    }
    return bindListner(server, listener);
};
exports.getServer = getServer;
const getHttpServer = function (_, listener) {
    const server = import_pkg_1.http2.createServer({
        maxSessionMemory: 128,
...
const request = function (req, res, callback) {

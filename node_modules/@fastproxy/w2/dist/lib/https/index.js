...
exports.setup = exports.handleWebsocket = void 0;
const tslib_1 = require("tslib");
const sni_1 = tslib_1.__importDefault(require("sni"));
const import_pkg_1 = require("../import-pkg");
const util = tslib_1.__importStar(require("../util"));
const config_1 = tslib_1.__importDefault(require("../config"));
const rules = tslib_1.__importStar(require("../rules"));
const plugins_1 = require("../plugins");
const socketMgr = tslib_1.__importStar(require("../socket-mgr"));
const util_1 = require("../rules/util");
const ca = tslib_1.__importStar(require("./ca"));
const load_cert_1 = tslib_1.__importDefault(require("./load-cert"));
const h2Consts = import_pkg_1.http2.constants;
...
const parseReq = import_pkg_1.hparser.parse;
...
const tunnelTmplData = new import_pkg_1.LRU({ max: 3000, maxAge: 30000 });
const TIMEOUT = 6000;
...
function handleWebsocket(socket, clientIp, clientPort) {
    ...
    plugins_1.pluginMgr.resolvePipePlugin(socket, function () {
...
function resolveWebsocket(socket, wss) {
    ...
    const reqEmitter = new import_pkg_1.EventEmitter();
        ...
        getResRules(socket, resData, function () {
            ...
            if (isSuccess) {
                ...
                if (!status) {
                    ...
                    resHeaders['Content-Length'] = import_pkg_1.Buffer.byteLength(body) + '';
                }
            ...
            if (socket.fromComposer) {
                curHeaders = (0, import_pkg_1.extend)({}, resHeaders);
    ...
    let plugin = plugins_1.pluginMgr.resolveWhistlePlugins(socket);
    abortIfUnavailable(socket);
    plugins_1.pluginMgr.getRules(socket, function (rulesMgr) {
        if (rulesMgr) {
            ...
            if (util.isIgnored(filter, 'rule')) {
                ...
                plugin = plugins_1.pluginMgr.getPluginByRuleUrl(util.rule.getUrl(_rules.rule));
            }
        }
        else {
            util.filterWeakRule(socket);
        ...
        if (util.showPluginReq(socket) && !socket.isInternalUrl) {
            ...
            if (config_1.default.captureData && (!filter.hide || socket.disable.hide)) {
                data.abort = destroy;
        ...
        setupSocket(function () {
            ...
            plugins_1.pluginMgr.loadPlugin(socket.isPluginReq ? null : plugin, function (err, ports) {
                if (plugin) {
                ...
                if (port) {
                    ...
                    plugins_1.pluginMgr.addRuleHeaders(socket, _rules);
                    socketMgr.setPending(socket);
                    options.protocol = 'ws:';
                    socket.headers[config_1.default.PLUGIN_HOOK_NAME_HEADER] =
                        config_1.default.PLUGIN_HOOKS.HTTP;
                    connectServer();
                    return;
                }
                plugin = null;
                    ...
                    rules.getProxy(fullUrl, socket, function (_err, hostIp, hostPort) {
                        ...
                        if (proxyUrl) {
                            ...
                            getServerIp(proxyUrl, function (ip) {
                                const proxyOptions = import_pkg_1.url.parse(proxyUrl);
                                proxyOptions.auth =
                                ...
                                const handleProxy = function (proxySocket) {
                                    if (wss) {
                                        ...
                                        try {
                                            proxySocket = import_pkg_1.tls.connect(opts);
                                ...
                                if (isInternalProxy && !socket._phost) {
                                    ...
                                    if (isSocks) {
                                        ...
                                        options.auths = config_1.default.getAuths(proxyOptions);
                                    }
                                    else {
                                        ...
                                        if (!util.isLocalAddress(clientIp)) {
                                            proxyHeaders[config_1.default.CLIENT_IP_HEAD] = clientIp;
                                        }
                                        if (isProxyPort) {
                                        ...
                                        const clientId = headers[config_1.default.CLIENT_ID_HEADER];
                                        if (clientId) {
                                    ...
                                    const netMgr = isSocks ? import_pkg_1.socks : config_1.default;
    ...
    function connectServer(hostIp, hostPort, ciphers) {
        getServerIp(fullUrl, function (ip, port) {
            ...
            try {
                ...
                reqSocket = isWss
                    ? import_pkg_1.tls.connect(opts, pipeData)
                    : import_pkg_1.net.connect(opts, pipeData);
            }
            catch (e) {
    ...
    function pipeData() {
        ...
        if (disable.clientIp || disable.clientIP) {
            ...
            else if (import_pkg_1.net.isIP(socket._customXFF)) {
                headers[config_1.default.CLIENT_IP_HEAD] = socket._customXFF;
            }
    ...
    function setupSocket(cb) {
        ...
        util.parseRuleJson(list, function (reqHeaders, reqCors, reqCookies, params, urlReplace, urlParams, auth) {
            if (params && urlParams) {
                (0, import_pkg_1.extend)(params, urlParams);
            }
            else {
            ...
            if (reqHeaders) {
                ...
                socket._customXFF = reqHeaders[config_1.default.CLIENT_IP_HEAD];
                delete reqHeaders[config_1.default.CLIENT_IP_HEAD];
                (0, import_pkg_1.extend)(headers, reqHeaders);
            ...
            plugins_1.pluginMgr.postStats(socket);
            cb();
        }, socket);
    }
    function getResRules(socket, res, callback) {
        ...
        plugins_1.pluginMgr.getResRules(socket, res, function () {
            util.parseRuleJson([_rules.resHeaders, _rules.resCors, _rules.resCookies], function (resHeaders, cors, cookies) {
                if (resHeaders) {
                    ...
                    (0, import_pkg_1.extend)(res.headers, resHeaders);
                }
        ...
        parseReq(reqSocket, function (err, res) {
            ...
            getResRules(socket, res, function () {
                util.delay(util.getMatcherValue(_rules.resDelay), function () {
                    ...
                    if (socket.fromComposer) {
                        curHeaders = (0, import_pkg_1.extend)({}, curResHeaders);
    ...
    function execCallback(err, _socket) {
        ...
        plugins_1.pluginMgr.postStats(socket, _socket || resData);
    }
}
function getTunnelData(socket, clientIp, clientPort, isHttpH2) {
    ...
    const disable = socket.disable || '';
    const headers = socket.headers;
    let tunnelData = headers[config_1.default.TUNNEL_DATA_HEADER];
    const tdKey = config_1.default.tdKey;
    if (tdKey && (!tunnelData || config_1.default.overTdKey)) {
        tunnelData = headers[tdKey] || tunnelData;
    }
    let tunnelHeaders;
    const tunnelKeys = plugins_1.pluginMgr.getTunnelKeys();
    ...
    return {
        ...
        clientId: headers[config_1.default.CLIENT_ID_HEADER],
...
function addReqInfo(req) {
    ...
    if (remoteData) {
        ...
        headers[config_1.default.CLIENT_INFO_HEAD] =
...
function toHttp1(req, res) {
    ...
    options.agent = false;
    options.host = config_1.default.host || LOCALHOST;
    options.port = config_1.default.port;
    options.headers = headers;
    if (isH2) {
        headers[config_1.default.ALPN_PROTOCOL_HEADER] = req.isHttpH2 ? 'httpH2' : 'h2';
    }
    options.method = req.method;
    client = import_pkg_1.http.request(options);
...
const h2Handlers = config_1.default.enableH2
    ? {
        request: toHttp1,
...
function addClientInfo(socket, chunk, statusLine, clientIp, clientPort) {
    const len = import_pkg_1.Buffer.byteLength(statusLine);
    chunk = chunk.slice(len);
    statusLine +=
        '\r\n' +
            config_1.default.CLIENT_INFO_HEAD +
            ': ' +
            clientIp +
            ',' +
            clientPort +
    ...
    if (tunnelData) {
        statusLine +=
            '\r\n' +
                config_1.default.TEMP_TUNNEL_DATA_HEADER +
                ': ' +
                encodeURIComponent(JSON.stringify(tunnelData));
    }
    return import_pkg_1.Buffer.concat([import_pkg_1.Buffer.from(statusLine), chunk]);
}
function default_1(socket, next, isWebPort) {
    let reqSocket;
    let reqDestroyed;
    let resDestroyed;
    let headersStr;
    ...
    util.readOneChunk(socket, function (chunk) {
        ...
        if (isHttp) {
            ...
            const handleRequest = function () {
                if (useSNI) {
                    checkTimeout();
                    let disableH2 = !util_1.properties.isEnableHttp2();
                    if (disable.http2) {
                        disableH2 = true;
                    }
            ...
            (0, load_cert_1.default)(socket, function (cert) {
                if (socket._hasError) {
                    return destroy();
                }
                if (cert === false) {

...
const file_mgr_1 = require("./util/file-mgr");
const common = tslib_1.__importStar(require("./util/common"));
const debug = (0, debug_1.default)('@w2.lib.config');
let customUIHost;
let customHostPluginMap = {};
const customPluginNameHost = {};
const WHISTLE_PLUGIN_RE = /^(?:whistle\.)?([a-z\d_\-]+)$/;
const CMD_RE = /^[\w]{1,12}(?:\s+-g)?$/;
const httpsAgents = new import_pkg_1.LRU({ max: 360 });
const socksAgents = new import_pkg_1.LRU({ max: 100 });
...
class Config {
    constructor() {
        this.version = import_pkg_1.pkgInfo.version;
        this.name = import_pkg_1.pkgInfo.name;
        this.dataDirname = import_pkg_1.pkgInfo.dataDirname;
        this.localUIHost = import_pkg_1.pkgInfo.localUIHost;
        this.sockets = import_pkg_1.pkgInfo.sockets;
        this.timeout = import_pkg_1.pkgInfo.timeout;
        this.registry = import_pkg_1.pkgInfo.registry;
        this.port = import_pkg_1.pkgInfo.port;
        this.uid = uid;
        this.rejectUnauthorized = false;
        this.enableH2 = majorVersion > 12; // 支持 HTTP2 要求的最低 Node 版本
        this.ASSESTS_PATH = import_pkg_1.path.join(__dirname, '../../assets');
        ...
        this.CUSTOM_PLUGIN_PATH = import_pkg_1.path.join(this.getWhistlePath(), 'custom_plugins');
        this.CUSTOM_CERTS_DIR = import_pkg_1.path.resolve(this.getWhistlePath(), 'custom_certs');
        this.pluginHostMap = {};
        ...
        this.getDataDir = (dirname) => {
            const dir = import_pkg_1.path.join(this.getWhistlePath(), dirname || '.' + this.name);
        ...
        this.getHttpsAgent = (options, reqOpts) => {
            ...
            if (!agent) {
                ...
                agent = new import_pkg_1.tunnel[agentName](options);
        ...
        this.extend = (newConf) => {
            ...
            if (newConf) {
                ...
                if (Array.isArray(dnsServer)) {
                    ...
                    dnsServer.forEach(function (ip) {
                        ...
                        else if (import_pkg_1.net.isIP(ip)) {
                            newServers.push(ip);
                        }
                        else if (IPV4_RE.test(ip) || IPV6_RE.test(ip)) {
                            const dnsIp = RegExp.$1;
                            const dnsPort = RegExp.$2;
                            if (import_pkg_1.net.isIP(dnsIp) && (!dnsPort || isPort(dnsPort.substring(1)))) {
                                newServers.push(ip);
                            }
                        }
                    });
                    if (newServers.length) {
                        import_pkg_1.dns.setServers(newServers);
                ...
                variableProperties.forEach(name => {
                    this[name] = newConf[name] || import_pkg_1.pkgInfo[name];
                });
                ...
                const resolveShadowValues = (obj) => {
                    if ((!obj.rules || typeof obj.rules === 'string') && obj.values && typeof obj.values !== 'string') {
                        ...
                        this.rules = (0, import_pkg_1.extend)({}, newConf.rules, obj);
                    }
                };
                if (typeof shadowRules === 'string') {
                    ...
                    if (typeof shadowRules[0] === 'string') {
                        ...
                        if (shadowRules[1]) {
                            this.rules = (0, import_pkg_1.extend)({}, newConf.rules, shadowRules[1]);
                        }
                        if (shadowRules[2]) {
                            this.values = (0, import_pkg_1.extend)({}, newConf.values, shadowRules[2]);
                        }
                    }
                    else if (shadowRules[0]) {
                        resolveShadowValues(shadowRules[0]);
                        if (shadowRules[1]) {
                            this.values = (0, import_pkg_1.extend)({}, newConf.values, shadowRules[1]);
                        }
                    }
                ...
                if (extra && typeof extra === 'string') {
                    ...
                    try {
                        ...
                        if (extra && typeof extra === 'object') {
                            this.pluginsDataMap = (0, import_pkg_1.extend)({}, this.pluginsDataMap, extra);
                        }
                    }
                    catch (e) { }
                    extra = null;
                ...
                if (newConf.certDir && typeof newConf.certDir === 'string') {
                    this.certDir = import_pkg_1.path.resolve(newConf.certDir);
                }
                ...
                if (typeof pluginHost === 'string' && (pluginHost = pluginHost.trim())) {
                    pluginHost = import_pkg_1.qs.parse(pluginHost);
            ...
            let baseDir = this.baseDir
                ? import_pkg_1.path.resolve(this.getHomePath(this.baseDir), this.dataDirname)
                : this.getDataDir(this.dataDirname);
            const customDirs = import_pkg_1.path.join(baseDir, 'custom_dirs');
            this.baseDir = baseDir;
            this.storage = this.storage && encodeURIComponent(this.storage);
            if (this.storage) {
                baseDir = import_pkg_1.path.join(customDirs, this.storage);
            }
            import_pkg_1.fse.ensureDirSync(baseDir);
            this.baseDirHash = createHash(baseDir);
            if (this.password) {
                this.passwordHash = createHash(this.password);
            }
            this.rulesDir = import_pkg_1.path.join(baseDir, 'rules');
            this.valuesDir = import_pkg_1.path.join(baseDir, 'values');
            this.propertiesDir = import_pkg_1.path.join(baseDir, 'properties');
            if (this.storage && newConf?.copy) {
                ...
                if (copyDir !== this.storage) {
                    const dataDir = copyDir ? import_pkg_1.path.join(customDirs, copyDir) : this.baseDir;
                    const rulesDir = import_pkg_1.path.join(dataDir, 'rules');
                    const valuesDir = import_pkg_1.path.join(dataDir, 'values');
                    const propsDir = import_pkg_1.path.join(dataDir, 'properties');
                    import_pkg_1.fse.ensureDirSync(rulesDir);
                    import_pkg_1.fse.ensureDirSync(valuesDir);
                    import_pkg_1.fse.ensureDirSync(propsDir);
                    import_pkg_1.fse.copySync(rulesDir, this.rulesDir);
                    import_pkg_1.fse.copySync(valuesDir, this.valuesDir);
                    import_pkg_1.fse.copySync(propsDir, this.propertiesDir);
                }
            }
            const clientIdFile = import_pkg_1.path.join(this.baseDir, '.clientid');
            let clientId = readFileText(clientIdFile);
            if (!clientId) {
                clientId = [Date.now(), Math.random(), Math.random(), Math.random(), Math.random(), Math.random()].join();
                import_pkg_1.fs.writeFileSync(clientIdFile, clientId);
            }
            this.clientId = clientId;
            this.LOCAL_FILES = import_pkg_1.path.join(this.baseDir, 'local_files');
            import_pkg_1.fse.ensureDirSync(this.LOCAL_FILES);
            if (!this.inited) {
        ...
        this.getSocksAgent = (options) => {
            ...
            (0, import_pkg_1.extend)(options, this.agentConfig);
            options.proxyPort = parseInt(options.proxyPort, 10) || 1080;
            options.rejectUnauthorized = this.rejectUnauthorized;
            options.localDNS = false;
            options.auths = this.getAuths(options);
            agent = options.isHttps ? new import_pkg_1.socks.HttpsAgent(options) : new import_pkg_1.socks.HttpAgent(options);
        ...
        this.getAuths = (_url) => {
            const options = typeof _url === 'string' ? import_pkg_1.url.parse(_url) : _url;
            if (!options || !options.auth) {
                return [import_pkg_1.socks.auth.None()];
            }
            const auths = [];
            ...
            return auths.length
                ? auths.map(function (auth) {
                    return import_pkg_1.socks.auth.UserPassword(auth.username, auth.password);
                })
                : [import_pkg_1.socks.auth.None()];
        };
        this.setAuth = (auth) => {
            if (auth) {
        ...
        this.connect = (options, cb) => {
            ...
            let httpModule = import_pkg_1.http;
            if (options.proxyServername) {
                proxyOptions.servername = options.proxyServername;
                httpModule = import_pkg_1.https;
            }
            let auth = options.proxyAuth || options.auth;
            if (auth) {
                auth = 'Basic ' + (0, file_mgr_1.toBuffer)(auth)?.toString('base64');
        ...
        try {
            import_pkg_1.fse.ensureDirSync(this.CUSTOM_CERTS_DIR);
        }
        catch (e) { }
    }
}
...
function createAgent(agentConfig, https) {
    const agent = new (https ? import_pkg_1.HttpsAgent : import_pkg_1.HttpAgent)(agentConfig);
    if (async_id_symbol) {
...
function resolvePath(file) {
    ...
    return /^[\w-]+$/.test(file) ? file : import_pkg_1.path.resolve(file);
}
function getHostname(_url) {
    ...
    if (_url.indexOf('/') != -1) {
        return import_pkg_1.url.parse(_url).hostname || '';
    }
...
function createHash(str) {
    const shasum = import_pkg_1.crypto.createHash('sha1');
    shasum.update(str);
    return shasum.digest('hex');
}
function readFileText(filepath) {
    try {
        return import_pkg_1.fs.readFileSync(filepath, { encoding: 'utf8' }).trim();
    }
    catch (e) {
        return void 0;
    }
...
function parseString(str) {
    ...
    if (/^\{[\s\S]+\}$/.test(str) || /^\[[\s\S]+\]$/.test(str)) {
        try {
            return import_pkg_1.json5.parse(str);
        }
        catch (e) { }
    }
    return str;

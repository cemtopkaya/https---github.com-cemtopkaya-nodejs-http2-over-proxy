...
const MAX_BODY_SIZE = 360 * 1024;
const MAX_SIZE = (config_1.default.strict ? 256 : 768) * 1024;
const MAX_REQ_BODY_SIZE = (config_1.default.strict ? 256 : 1536) * 1024;
const MAX_RES_BODY_SIZE = (config_1.default.strict ? 256 : 1536) * 1024;
const BIG_DATA_SIZE = 2560 * 1024;
...
function emitDataEvents(req, res, proxy) {
    const now = Date.now();
    const eventName = getEventName(proxy);
    eventName && proxy.emit(eventName, req.fullUrl);
    if (!util.showPluginReq(req) ||
        !config_1.default.captureData ||
    ...
    const useBigData = (enable.bigData || enable.largeData) &&
        !disable.bigData &&
        !disable.largeData;
    ...
    const setUnzipSize = function (body, obj) {
        const len = body ? body.length : -1;
    ...
    let reqDone;
    let resDone;
    const handleReqBody = function (stream, info) {
        if (reqDone) {
            return;
        }
        reqDone = true;
    ...
    const handleResBody = function (stream, info) {
        if (resDone) {
            return;
        }
        resDone = true;
        ...
        const useStream = isStream && !getZipType(info);
        const MAX_RES_BODY = useBigData
            ? BIG_DATA_SIZE
            : isUnzipJs(info)
                ? MAX_RES_BODY_SIZE
                : MAX_SIZE;
    ...
    const hasReqPipe = req._pipePluginPorts.reqWritePort;
    const hasResPipe = req._pipePluginPorts.resWritePort;
    if (hasReqPipe) {
...
function inspectData(req, res, next) {
...
exports.inspectData = inspectData;

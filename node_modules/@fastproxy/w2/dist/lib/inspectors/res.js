...
const TIMEOUT = config_1.default.timeout < 16000 && config_1.default.timeout > 0 ? 0 : 16000;
const LOCALHOST = '127.0.0.1';
const DOCTYPE = util.toBuffer('<!DOCTYPE html>\r\n');
const CRLF = util.toBuffer('\r\n');
const MAX_RES_SIZE = 512 * 1024;
const JSON_RE = /{[\w\W]*}|\[[\w\W]*\]/;
const LIKE_JSON_RE = /^\s*[\{\[]/;
const X_RE = /^x/;
const BODY_PROTOCOLS = [
    'attachment',
    'resReplace',
    'resBody',
    'resPrepend',
...
const BODY_PROTOCOLS_LEN = BODY_PROTOCOLS.length;
function notAllowCache(resRules) {
    for (let i = 0; i < BODY_PROTOCOLS_LEN; i++) {
...
function setCookies(headers, data) {
    let newCookies = data.headers['set-cookie'];
    ...
    if (newCookies.length) {
        ...
        let isArray = Array.isArray(cookies);
        if (!isArray && cookies) {
            isArray = true;
        ...
        if (isArray) {
            ...
            newCookies.forEach(function (cookie) {
                ...
                const name = index == -1 ? undefined : cookie.substring(0, index);
                newNameMap[name] = 1;
            });
...
function handleReplace(res, replacement) {
    ...
    const type = util.getContentType(res.headers);
    if (!type || type == 'IMG') {
        return;
    }
    Object.keys(replacement).forEach(function (pattern) {
        ...
        if (util.isOriginalRegExp(pattern) && (pattern = util.toOriginalRegExp(pattern))) {
            res.addTextTransform(new util_1.ReplacePatternTransform(pattern, value));
        }
        else if (pattern) {
            res.addTextTransform(new util_1.ReplaceStringTransform(pattern, value));
        }
    });
}
...
function readFirstChunk(req, res, src, cb) {
    const ports = req._pipePluginPorts;
    if (!cb) {
...
function inspectRes(req, res, next) {
    let origProto;
    const resRules = req.rules;
    const deleteHeaders = util.parseDeleteProperties(req);
    req.request = function (options) {
        readFirstChunk(req, function () {
            ...
            const originPort = options.port;
            const originHost = options.host;
            const now = Date.now();
            rules.getClientCert(req, function (key, cert, isPfx, cacheKey) {
                rules.getProxy(options.href, options.isPlugin ? null : req, function (_err, hostIp, hostPort) {
                    const proxyRule = resRules.proxy;
                    let proxyUrl = !options.isPlugin && proxyRule ? util.rule.getMatcher(proxyRule) : null;
                    ...
                    let origProxy;
                    if (!hostIp) {
                        if (options.localDNS && import_pkg_1.net.isIP(options.host)) {
                            curUrl = options.host;
                        }
                        else if (proxyUrl) {
                            ...
                            if (isInternalProxy) {
                                ...
                                if (options.protocol === 'https:') {
                                    headers[config_1.default.HTTPS_FIELD] = 1;
                    ...
                    rules.resolveHost(req, function (err, ip, port, hostRule) {
                        const setHostsInfo = function (_ip, _port, _host, withPort) {
                            ip = _ip || '127.0.0.1';
                            port = _port;
                        ...
                        let isProxyPort;
                        let isSocks;
                        const setAgent = function (disable) {
                            if (disable || req.disable.keepAlive || (isHttps && cert)) {
                                options.agent = false;
                            }
                            else {
                                options.agent = isHttps ? config_1.default.httpsAgent : config_1.default.httpAgent;
                            }
                        };
                        checkH2(req, isHttps);
                        if (proxyUrl) {
                            proxyOptions = import_pkg_1.url.parse(proxyUrl);
                            proxyOptions.host = ip;
                            proxyOptions.auth = proxyOptions.auth || req._pacAuth;
                            isSocks = proxyRule.isSocks;
                            let proxyPort = proxyOptions.port;
                            if (!proxyPort) {
                            ...
                            if (isHttps || (req.useH2 && !isInternalProxy) || isSocks || isHttpsProxy || req._phost) {
                                isProxyPort = util.isProxyPort(proxyPort);
                                if (isProxyPort && util.isLocalAddress(ip)) {
                                    req.setServerPort(config_1.default.port);
                                    res.response(util.wrapResponse({
                                        statusCode: 302,
                                        headers: {
                                            location: 'http://' + util.getHostIp(ip, config_1.default.port) + (options.path || '')
                                        }
                                    }));
                                }
                                else {
                                    const curServerPort = options.port || (isHttps ? 443 : 80);
                                    const proxyHeaders = {
                                        host: options.hostname + ':' + curServerPort,
                                        'proxy-connection': req.disable.proxyConnection ? 'close' : 'keep-alive'
                                    };
                                    plugins_1.pluginMgr.getTunnelKeys().forEach(function (k) {
                                        const val = headers[k];
                                        if (val) {
                                            proxyHeaders[k] = val;
                                        }
                                    ...
                                    const opts = {
                                        ...
                                        proxyHost: ip,
                                        clientIp: proxyHeaders[config_1.default.CLIENT_IP_HEAD],
                                        proxyPort: proxyPort,
                                        url: options.href,
                        ...
                        const curPort = port || (isHttps ? 443 : 80);
                        isProxyPort = util.isProxyPort(curPort);
                        const isLocalAddress = util.isLocalAddress(options.host);
                        if (isProxyPort && isLocalAddress) {
                            const redirectHost = config_1.default.customLocalUIHost || ip;
                            const redirectPort = config_1.default.realPort || config_1.default.port;
                            res.response(util.wrapResponse({
                                statusCode: 302,
                                headers: {
                        ...
                        function request(options, serverPort, direct) {
                            ...
                            options.rejectUnauthorized = config_1.default.rejectUnauthorized;
                            ...
                            let curClient;
                            let timer;
                            const setProxyAgent = function (options, proxyOpts) {
                                ...
                                options.agent = proxyOpts.isSocks
                                    ? config_1.default.getSocksAgent(proxyOpts)
                                    : config_1.default.getHttpsAgent(proxyOpts, options);
                            };
                            const retry = function (err) {
                                ...
                                if (retryCount >= maxRetryCount) {
                                    ...
                                    if (!toHttp &&
                                        (resetCount > 1 || (code !== 'EPROTO' && code !== 'ECONNRESET') || (piped && !req.noReqBody))) {
                                        const stack = util.getErrorStack(err || new Error('socket connect timeout'));
                            ...
                            const send = function (sock) {
                                ...
                                if (origPath != null && proxyUrl) {
                                    origPath = undefined;
                                    options.path =
                                        (isHttps ? 'https:' : 'http:') + '//' + (headers.host || options.host) + (options.path || '/');
                                }
                                let useHttps = isHttps;
                                if (sock) {
                                    options.agent = null;
                                ...
                                try {
                                    const client = (useHttps ? import_pkg_1.https : import_pkg_1.http).request(options, res.response);
                                    curClient = client;
                            ...
                            if (req.disable.clientIp || req.disable.clientIP) {
                                ...
                                const forwardedFor = util.getMatcherValue(resRules.forwardedFor);
                                if (import_pkg_1.net.isIP(forwardedFor)) {
                                    headers[config_1.default.CLIENT_IP_HEAD] = forwardedFor;
                                }
                                else if (import_pkg_1.net.isIP(req._customXFF)) {
                                    headers[config_1.default.CLIENT_IP_HEAD] = req._customXFF;
                                }
                                    ...
                                    headers[config_1.default.CLIENT_IP_HEAD] = req.clientIp;
                                }
                            }
                            ...
                            const transfer = options.method === 'DELETE' && optHeaders['transfer-encoding'];
                            if (transfer) {
                            ...
                            if (proxyUrl) {
                                ...
                                const clientId = optHeaders[config_1.default.CLIENT_ID_HEADER];
                                if (clientId) {
                            ...
                            if (req.useH2 && (isInternalProxy || headers[config_1.default.HTTPS_FIELD] || options.isPlugin)) {
                                headers[config_1.default.ALPN_PROTOCOL_HEADER] = 'h2';
                            }
                            ...
                            delete headers[config_1.default.ALPN_PROTOCOL_HEADER];
                            if (transfer) {
                                optHeaders['Transfer-Encoding'] = transfer;
                            }
                            if (options.isPlugin) {
                                optHeaders[config_1.default.PLUGIN_HOOK_NAME_HEADER] = config_1.default.PLUGIN_HOOKS.HTTP;
                            }
                            req.noReqBody = !util.hasRequestBody(req);
                            if (req.method === 'DELETE' &&
                                (req._hasInjectBody || req.headers['transfer-encoding'] || req.headers['content-length'] > 0)) {
                                req.useH2 = false;
                            }
                            req.setServerPort(options._proxyPort || options.port || (isHttps ? 443 : 80));
                            req.options = options;
    ...
    res.response = function (_res) {
        ...
        if (!req.isPluginReq && headers[config_1.default.PROXY_ID_HEADER] === 'h2') {
            req.useH2 = true;
            delete headers[config_1.default.PROXY_ID_HEADER];
        }
        util.drain(req, function () {
            readFirstChunk(req, res, _res, function (firstChunk) {
                plugins_1.pluginMgr.getResRules(req, _res, function () {
                    const replaceStatus = util.getMatcherValue(resRules.replaceStatus);
                    ...
                    util.parseRuleJson(ruleList, function (headers, cookies, cors, replacement, params, newTrailers) {
                        ...
                        const cache = util.getMatcherValue(resRules.cache);
                        const enable = req.enable;
                        if (cache === 'reserve' || enable.keepAllCache) {
                            req._customCache = true;
                        }
                        else {
                            const maxAge = parseInt(cache, 10);
                            const noCache = /^(?:no|no-cache|no-store)$/i.test(cache) || maxAge < 0;
                            if (maxAge > 0 || noCache) {
                                req._customCache = true;
                                util.setHeaders(data, {
                                    'cache-control': noCache
                                        ? /^no-store$/i.test(cache)
                                            ...
                                            : 'no-cache'
                                        : 'max-age=' + maxAge,
                                    expires: new Date(Date.now() + (noCache ? -60000000 : maxAge * 1000)).toUTCString(),
                                    pragma: noCache ? 'no-cache' : ''
                                });
                            }
                        }
                        if (resRules.attachment) {
                            const attachment = util.getMatcherValue(resRules.attachment) || util.getFilename(req.fullUrl);
                            util.setHeader(data, 'content-disposition', 'attachment; filename="' + util.encodeNonLatin1Char(attachment) + '"');
                        }
                        if (resRules.resType) {
                            const newType = util.getMatcherValue(resRules.resType).split(';');
                            const type = newType[0];
                            newType[0] = !type || type.indexOf('/') != -1 ? type : import_pkg_1.mime.lookup(type, type);
                            util.setHeader(data, 'content-type', newType.join(';'));
                        }
                        if (resRules.resCharset) {
                            data.charset = util.getMatcherValue(resRules.resCharset);
                        }
                        let resSpeed = util.getMatcherValue(resRules.resSpeed);
                        ...
                        util.readInjectFiles(data, function (data) {
                            ...
                            if (data.headers) {
                                ...
                                if (typeof type === 'string') {
                                    if (type.indexOf(';') == -1) {
                                        let origType = headers['content-type'];
                                ...
                                (0, import_pkg_1.extend)(headers, customHeaders);
                            }
                            if (data.charset && typeof data.charset === 'string') {
                                type = headers['content-type'];
                                type = typeof type === 'string' ? type.trim().split(';')[0] : '';
                                type = type || import_pkg_1.mime.lookup(req.fullUrl.replace(/[?#].*$/, ''), 'text/html');
                            ...
                            const hr = util.parseHeaderReplace(resRules.headerReplace);
                            ...
                            const charset = util.getCharset(headers['content-type']);
                            let isHtml = type === 'HTML';
                            const isJs = isHtml || type === 'JS';
                            const isCss = isHtml || type === 'CSS';
                            const hasResBody = util.hasBody(_res, req);
                            util.getRuleValue([
                                resRules.resBody,
                                resRules.resPrepend,
                                resRules.resAppend,
                            ...
                            ], function (resBody, resPrepend, resAppend, htmlAppend, jsAppend, cssAppend, htmlBody, jsBody, cssBody, htmlPrepend, jsPrepend, cssPrepend) {
                                if (req._hasError) {
                                    return;
                                }
                                if (resBody != null) {
                                ...
                                const speedTransform = data.speed || data.delay ? new util_1.SpeedTransform(data) : null;
                                delete data.headers;
                                delete data.speed;
                                delete data.delay;
                                ...
                                if ((isHtml || type === 'JSON' || type === 'JS') && params && Object.keys(params).length) {
                                    ...
                                    let interrupt;
                                    let ctn = '';
                                    transform._transform = function (text, _encoding, callback) {
                                        if (text) {
                                            if (!interrupt) {
                                                ctn += text;
                                                text = null;
                                            ...
                                            text = ctn.replace(JSON_RE, function (json) {
                                                ...
                                                return obj ? JSON.stringify((0, import_pkg_1.extend)(true, obj, params)) : json;
                                            });
                                            ctn = '';
                                        }
                                        else if (!interrupt) {
                                ...
                                const hasData = data.body || data.top || data.bottom;
                                if (hasData) {
                                ...
                                if (hasData || util.isWhistleTransformData(data)) {
                                    ...
                                    res.addZipTransform(new util_1.WhistleTransform(data));
                                }
                                if (hasResBody) {
                                ...
                                const bodyFile = hasResBody
                                    ? getWriterFile(util.getRuleFile(resRules.resWrite), _res.statusCode)
                                    : null;
                                const rawFile = getWriterFile(util.getRuleFile(resRules.resWriteRaw), _res.statusCode);
                                util.getFileWriters([bodyFile, rawFile], function (writer, rawWriter) {
                                    ...
                                    res.on('src', function (_res) {
                                        if (writer) {
                                            res.addZipTransform(new util_1.FileWriterTransform(writer, _res));
                                        }
                                        if (rawWriter) {
                                            res.addZipTransform(new util_1.FileWriterTransform(rawWriter, _res, true, req));
                                        }
                                    });
                                    const delProps = util.parseDeleteProperties(req);
                                    const resHeaders = delProps.resHeaders;
                                    Object.keys(resHeaders).forEach(function (prop) {
                                        delete headers[prop];
                                    });
                                    if (headers[config_1.default.ALPN_PROTOCOL_HEADER] === 'h2') {
                                        req.useH2 = true;
                                    }
                                    util.delay(util.getMatcherValue(resRules.resDelay), function () {
                                        ...
                                        const rawNames = res.rawHeaderNames || {};
                                        if (enable.gzip) {
                                        ...
                                        plugins_1.pluginMgr.postStats(req, res);
                                        ...
                                        if (req.fromComposer) {
                                            curHeaders = (0, import_pkg_1.extend)({}, headers);
                                        ...
                                        try {
                                            ...
                                            util.onResEnd(_res, function () {
                                                ...
                                                const delTrailers = delProps.trailers;
    ...
    const resHeaders = {};
    const svRes = util.getStatusCodeFromRule(resRules);
    if (svRes) {
